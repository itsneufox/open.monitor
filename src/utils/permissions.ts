...
import { ChatInputCommandInteraction, PermissionFlagsBits, MessageFlags, GuildMember } from 'discord.js';
import { CustomClient } from '../types';

export async function hasManagementPermission(interaction: ChatInputCommandInteraction, client: CustomClient): Promise<boolean> {
...
export async function checkPermissionOrReply(interaction: ChatInputCommandInteraction, client: CustomClient): Promise<boolean> {
  ...
  if (!hasPermission) {
    ...
    if (!intervalConfig?.managementRoleId) {
      errorMessage += 'Only server administrators can use this command.\n\n💡 **Tip**: An admin can use `/role set` to allow a specific role to manage the bot.';
    } else {

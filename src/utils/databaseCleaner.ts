...
class DatabaseCleaner {
  ...
  async cleanupServer(serverId: string): Promise<{ success: boolean; errors: string[] }> {
  ...
  async cleanupGuild(guildId: string): Promise<{ success: boolean; serversProcessed: number; errors: string[] }> {
  ...
  async cleanupOldChartData(): Promise<{ serversProcessed: number; dataPointsRemoved: number; errors: string[] }> {
      ...
      for (const server of guildConfig.servers) {
        try {
          ...
          if (chartData?.days) {
            ...
            chartData.days = chartData.days.filter(day => day.date > thirtyDaysAgo);
            ...
            if (originalLength > newLength) {
              ...
              console.log(`  🗑️  Removed ${originalLength - newLength} old data points from ${server.id}`);
            }
          }
          serversProcessed++;
        } catch (error) {
  ...
  async runPeriodicCleanup(): Promise<{ summary: string; errors: string[] }> {
    ...
    if (chartResult.errors.length > 0) {
      console.warn(`⚠️  Cleanup completed with ${chartResult.errors.length} errors`);
    } else {

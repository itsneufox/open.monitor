...
export async function generateChart(
  data: ChartData | ChartDataWrapper,
  color: number
): Promise<AttachmentBuilder> {
  const chartData = (data as ChartDataWrapper).value
  ...
  console.log('Chart debug info:', {
    ...
    daysLength: (chartData as ChartData).days?.length,
    firstFewDays: (chartData as ChartData).days?.slice(0, 3),
    serverName: (chartData as ChartData).name,
  });

  if (
    !(chartData as ChartData).days ||
    (chartData as ChartData).days.length === 0
  ) {
    throw new Error('No chart data available');
  }

  const sortedDays = [...(chartData as ChartData).days].sort(
    (a, b) => a.date - b.date
  );
  ...
  console.log('Processed chart data:', {
    ...
    dateRange: {
      ...
      last: new Date(sortedDays[sortedDays.length - 1]?.date || 0),
    },
    valueRange: {
      min: Math.min(...sortedDays.map(d => d.value)),
      max: Math.max(...sortedDays.map(d => d.value)),
    },
    sampleValues: sortedDays
      .slice(0, 5)
      .map(d => ({ value: d.value, date: new Date(d.date) })),
  });
  ...
  const values = sortedDays.map(day => Math.max(0, day.value));

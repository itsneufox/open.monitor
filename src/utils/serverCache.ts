import { ServerConfig, ServerMetadata, getServerDataKey, CustomClient } from '../types';
import { SAMPQuery } from './sampQuery';

export class ServerMetadataCache {
    private static cache = new Map<string, ServerMetadata>();
    private static readonly METADATA_CACHE_DURATION = 24 * 60 * 60 * 1000; // 24 hours
    ...
    static async getMetadata(
        ...
        if (cached && Date.now() - cached.lastUpdated < this.METADATA_CACHE_DURATION) {
            return cached;
        }
        ...
        if (client) {
            try {
                ...
                if (dbCached && Date.now() - dbCached.lastUpdated < this.METADATA_CACHE_DURATION) {

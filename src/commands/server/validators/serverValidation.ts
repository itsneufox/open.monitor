...
export function validateServerInput(
  ...
  if (name) {
    ...
    if (!nameValidation.valid) {
      return {
        valid: false,
        error: `Server name invalid: ${nameValidation.error!}`,
      };
    }
  ...
  if (!ipValidation.valid) {
    return {
      valid: false,
      error: `IP address invalid: ${ipValidation.error!}`,
    };
  }
  ...
  if (!TimezoneHelper.validateGMT(timezone)) {
    return {
      valid: false,
      error:
        'Invalid timezone format. Use GMT+X or GMT-X (e.g., GMT+2, GMT-5, GMT+0).',
    };
  }
  ...
  if (ip.startsWith('127.') || ip === '::1') {
    return {
      valid: false,
      error: 'Localhost addresses (127.x.x.x and ::1) are not allowed.',
    };
  }
...
export async function canQueryServer(
  ip: string,
  guildId: string
): Promise<boolean> {
  const result = await SecurityValidator.canQueryIP(ip, guildId);
...
export function validateServerExists(
  ...
  servers: any[]
): { valid: boolean; server?: any; error?: string } {
  if (!serverIdentifier) {
  ...
  if (!server) {
    return {
      valid: false,
      error: 'Server not found. Use `/server list` to see available servers.',
    };
  }

...
export async function handleSetup(
  ...
  const autoTextChannels =
    interaction.options.getBoolean('auto_text_channels') ?? true;
  const autoVoiceChannels =
    interaction.options.getBoolean('auto_voice_channels') ?? true;
  const createCategory =
    interaction.options.getBoolean('create_category') ?? true;
  const enableMonitoring =
    interaction.options.getBoolean('enable_monitoring') ?? true;

  const manualStatusChannel = interaction.options.getChannel(
    'status_channel'
  ) as TextChannel | null;
  const manualChartChannel = interaction.options.getChannel(
    'chart_channel'
  ) as TextChannel | null;
  const manualPlayerCountChannel = interaction.options.getChannel(
    'player_count_channel'
  ) as VoiceChannel | null;
  const manualServerIpChannel = interaction.options.getChannel(
    'server_ip_channel'
  ) as VoiceChannel | null;
  ...
  if (
    ...
    !manualStatusChannel &&
    !manualChartChannel &&
    !manualPlayerCountChannel &&
    !manualServerIpChannel
  ) {
    await interaction.editReply(
      'You must either enable auto-creation or provide at least one channel manually.'
    );
    return;
  }
  ...
  try {
    ...
    if (createCategory) {
      let monitorCategory = interaction.guild!.channels.cache.find(
        ch =>
          ch.type === ChannelType.GuildCategory && ch.name === 'open.monitor'
      );
      if (!monitorCategory) {
    ...
    if (autoTextChannels) {
      ...
      const createdStatusChannel =
        await interaction.guild!.channels.create(statusChannelOptions);
      ...
      const createdChartChannel =
        await interaction.guild!.channels.create(chartChannelOptions);
    ...
    if (autoVoiceChannels) {
      ...
      const createdPlayerCountChannel =
        await interaction.guild!.channels.create(playerCountChannelOptions);
      ...
      const createdServerIpChannel = await interaction.guild!.channels.create(
        serverIpChannelOptions
      );
    ...
    const channelsToCheck = [
      {
        channel: finalStatusChannel,
        perms: ['ViewChannel', 'SendMessages', 'EmbedLinks'],
      },
      {
        channel: finalChartChannel,
        perms: ['ViewChannel', 'SendMessages', 'AttachFiles'],
      },
      {
        channel: finalPlayerCountChannel,
        perms: ['ViewChannel', 'ManageChannels'],
      },
      {
        channel: finalServerIpChannel,
        perms: ['ViewChannel', 'ManageChannels'],
      },
    ];
    ...
    if (finalPlayerCountChannel)
      newConfig.playerCountChannel = finalPlayerCountChannel.id;
    if (finalServerIpChannel)
      newConfig.serverIpChannel = finalServerIpChannel.id;
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: 'Status Channel',
          value: finalStatusChannel?.toString() || 'Not configured',
          inline: true,
        },
        {
          name: 'Chart Channel',
          value: finalChartChannel?.toString() || 'Not configured',
          inline: true,
        },
        {
          name: 'Monitoring',
          value: enableMonitoring ? 'Enabled' : 'Disabled',
          inline: true,
        }
      )
      .setTimestamp();
    ...
    if (finalPlayerCountChannel) {
      embed.addFields({
        name: 'Player Count',
        value: finalPlayerCountChannel.toString(),
        inline: true,
      });
    }
    if (finalServerIpChannel) {
      embed.addFields({
        name: 'Server IP',
        value: finalServerIpChannel.toString(),
        inline: true,
      });
    }
    ...
    if (!finalStatusChannel && enableMonitoring) {
      embed.addFields({
        ...
        value:
          'Monitoring enabled but no status channel configured. Only voice channels will update.',
        inline: false,
      });
    }
    ...
    await interaction.editReply(
      'An error occurred while setting up monitoring channels. Please check bot permissions.'
    );
  }
}

...
async function handleAdd(
  ...
  if (name) {
    ...
    if (!nameValidation.valid) {
      await interaction.editReply(`❌ Server name invalid: ${nameValidation.error}`);
      return;
    }
  ...
  try {
    ...
    const finalServerName = serverName || testResult?.hostname || `${ip}:${port}`;

    // Validate the final server name (from server response)
    if (testResult?.hostname) {
      const hostnameValidation = InputValidator.validateServerName(testResult.hostname);
      ...
      if (intervalConfig.serverIpChannel) {
        try {
          ...
          if (serverIpChannel && 'setName' in serverIpChannel) {
            const channelNameValidation = InputValidator.validateChannelName(`Server ${ip}:${port}`);
   ...
   console.log(`✅ Server added: ${finalServerName} (${ip}:${port}) by ${interaction.user.tag} in guild ${interaction.guild?.name}`);
...
async function handleRemove(
 ...
 try {
   ...
   const embed = new EmbedBuilder()
     ...
     .setDescription(`Removed **${server.name}** and cleaned up all associated data`)
     .addFields(
       {
         name: 'Server Address',
         value: `${server.ip}:${server.port}`,
